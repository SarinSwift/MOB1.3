...
func printNameAndAge(person: Guest) {
    print("\(person.name), \(person.age)")
}

//call the function and pass in eric into the function
printNameAndAge(person: eric)
...
func eachGuest(array: [Guest]) {
    for guest in array {
//        printNameAndAge(person: guest)
        print(guest.name)
    }
}
...
func olderThanEighteen(array: [Guest]) -> [Guest] {
    var answerArray = [Guest]()
    
    for guest in array {
        if guest.age >= 18 {
            answerArray.append(guest)
        }
    }
...
olderThanEighteen(array: invitedGuests)
...
/*:
 ...
 3. As used in our `Guest` struct, this property is part of the `CustomDebugStringConvertible` protocol. Here we return the value of an instance method `getFullName() -> String`
 4. This is our **initializer, or constructor,** for our class. Since we need to update the number of customers every time we get a new one, it makes the most sense to increament that count during the creation process of a customer.
 5. A **deinit** is the opposite of an initalizer. This block of code is executed when an instance of our class is being deleted from memory. More on memory management in a later lesson. Here, we'll decrement the customer counter since we need to keep track of removed customers.
 ...
 7. This is our **instance method**, which is a method that returns a value that is unique to each instance of our class (unique to each customer). This method returns the first and last name values into a single string separated by a space. Each customer will have a unique return value when they call this method (unless they have the same name).
