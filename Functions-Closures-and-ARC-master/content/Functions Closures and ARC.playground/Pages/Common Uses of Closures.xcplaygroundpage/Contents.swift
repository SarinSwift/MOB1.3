...
let adultListFIlter = guestList.filter { (guest) -> Bool in
    if guest.age >= 18 {
        return true
    }
    return false
...
let sortedByAge = guestList.sorted { (currentGuest, nextGuest) -> Bool in
    return nextGuest.age < currentGuest.age
}
...
let upperCases = lowerCaseLetters.map { (letter) -> String in
...
let agesOfGuests = guestList.map { (guest) -> Int in
    return guest.age
}

let names = guestList.map { (guest) -> String in
    return guest.name
}
...
let sumNumbers = randomNumbers.reduce(0) { (current, next) -> Int in
    return current + next
}
...
let sortedNumbers = numbersToSort.sorted { (first, second) -> Bool in
    first < second
}
print(sortedNumbers)
...
let sortedNames = guestsToSort.sorted { (guessA, guessB) -> Bool in
    return guessA.name < guessB.name
}
print(sortedNames)

//sort the guests by age, but in descending order (youngest at the front of the array)

let descendingAge = guestsToSort.sorted { (guessA, guessB) -> Bool in
    return guessA.age < guessB.age
}

//filter the guests to only include guests younger than 18 years

let youngerThanEighteenFilter = guestsToSort.filter { (guestA) -> Bool in
...
let doubleValues = numbersToDouble.map{ (num) -> Int in
    return num * 2
}
...
let stringNumbers = numbersToMapIntoStrings.map{ (num) -> String in
    return String(num)
}
...
let reduceClosure = numbersToSumm.reduce(0) { (result, num) -> Int in
    if num >= 0 {
        return result + num
    }
    return result
