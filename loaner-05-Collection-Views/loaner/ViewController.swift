...
class ViewController: UIViewController {
    ...
    @IBAction func unwindToHome(_ segue: UIStoryboardSegue) {
        ...
        case "unwind from saving new item":
            ...
            // DIDDO: uncommenting this part out because the data is saved in firebase database!!
//            add(saved: itemContactVc.item)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //DIDDO: retreive data from the database
        var ref: DatabaseReference!
        ref = Database.database().reference().child("items")
        ref.observe(DataEventType.value) { (snapshot) in
            
            print("snapshot issss: \(snapshot)")
            self.items = []
            ...
            for rest in postDict {
                ...
                guard let name = restDict["name"],
                    ...
                    let url = restDict["imageUrl"] else { return }
...
extension ViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // DIDDO: Setting each cell's image to the url from firebase storage
        let url = URL(string: item.imageUrl!)
        do {
            let data = try Data(contentsOf: url!)
            cell.imageView.image = UIImage.init(data: data)
        } catch {
            print("error setting image to cell")
        }
